...
class DataController extends Controller
{
    ...
    public function uploadFile(Request $request)
    {
        ...
        foreach ($data as $sheetIndex => $rows) {
            ...
            foreach ($headers as $index => $header) {
                if (empty($header)) continue; // Skip empty headers
    ...
    {
        // For now, keep the old logic or adapt for session data
        // If session data exists, print that.
        $results = Session::get('analysis_results', []);

        if (!empty($results)) {
             $pdf = Pdf::loadView('pdf.report_generic', compact('results'))
