    ...
    @if(Session::has('hypothesis_results'))
        ...
        @foreach(Session::get('hypothesis_results') as $index => $item)
            <div style="margin-bottom: 30px; border-bottom: 1px solid #eee; padding-bottom: 20px;">
                ...
                <!-- If we captured hypothesis chart, we would need a unique ID for it. 
                     In results.blade.php, there's only ONE active chart canvas 'activeHypothesisChart'. 
                     This makes capturing ALL hypothesis charts tricky unless we render them all or capture the active one.
                     Plan: For now, we might skip hypothesis charts in PDF or only capture the currently visible one?
                     User wants "Report", so maybe just text is fine for hypothesis if charts are dynamic.
                     OR we can try to render them, but it's hard. Let's stick to text for Hypothesis for now to be safe.
                -->
            </div>
        @endforeach
    @endif
