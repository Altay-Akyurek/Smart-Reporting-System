...
<head>
    ...
    <style>
        ...
        /* Print Mode Optimization (Native Browser Print) */
        @media print {
            ...
            .sidebar, .btn-glow, .reveal.active > div > button, .dropdown, #techDetails button {
            ...
            h1, h2, h3, h4, h5, h6, .text-white, .metric-value, .fw-bold {
                color: black !important;
            }
            .text-dim, .text-mute {
                color: #555 !important;
            ...
            /* Specific fix for Correlation Matrix text in print */
            .corr-cell {
                color: black !important; 
        ...
        /* Action Buttons */
        .btn-glow {
            background: linear-gradient(135deg, var(--active-glow), #8b5cf6);
            color: #fff;
            border: none;
            padding: 12px 32px;
            border-radius: 50px;
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="d-flex justify-content-between align-items-center mb-5 reveal active">
            <div>
                ...
                <p class="text-dim">İstatistik Destekli İçgörüler</p>
            </div>
            <!-- PDF Button -->
            <button onclick="preparePdfDownload()" class="btn-glow" id="btnPdf">
                <span id="btnPdfText">📥 PDF Rapor İndir</span>
                <span id="btnPdfSpinner" class="spinner-border spinner-border-sm d-none" role="status" aria-hidden="true"></span>
            </button>
            <form id="pdfForm" action="{{ route('downloadPdf') }}" method="POST" class="d-none">
                @csrf
                            ...
                            <div class="d-flex justify-content-between align-items-center mb-2">
                                ...
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end dropdown-menu-dark p-2" style="min-width: 200px;">
                                        ...
                                        <li><a class="dropdown-item active" href="javascript:void(0)" onclick="changeChartType('curve', '📈 Çan Eğrisi (Normal Dağılım)')">📈 Çan Eğrisi (Normal Dağılım)</a></li>
                                        <li><a class="dropdown-item" href="javascript:void(0)" onclick="changeChartType('histogram', '📊 Histogram (Frekans)')">📊 Histogram (Frekans)</a></li>
                                        <li><a class="dropdown-item" href="javascript:void(0)" onclick="changeChartType('scatter', '✨ Dağılım (Scatter Plot)')">✨ Dağılım (Scatter Plot)</a></li>
                                        <li><a class="dropdown-item" href="javascript:void(0)" onclick="changeChartType('box', '📦 Kutu Grafiği (Box Plot)')">📦 Kutu Grafiği (Box Plot Sim.)</a></li>
                                        
                                        <li><hr class="dropdown-divider border-secondary"></li>
                                        <li><h6 class="dropdown-header text-dim">Karşılaştırma</h6></li>
                                        <li><a class="dropdown-item" href="javascript:void(0)" onclick="changeChartType('bar', '📊 Sütun Grafik (Ortalama)')">📊 Sütun Grafik (Ortalama)</a></li>
                                        <li><a class="dropdown-item" href="javascript:void(0)" onclick="changeChartType('horizontalBar', '➖ Yatay Sütun (Ortalama)')">➖ Yatay Sütun (Ortalama)</a></li>
                                        <li><a class="dropdown-item" href="javascript:void(0)" onclick="changeChartType('radar', '🕸️ Radar Analizi (Özet)')">🕸️ Radar Analizi (Özet)</a></li>
                                        <li><a class="dropdown-item" href="javascript:void(0)" onclick="changeChartType('polar', '❄️ Polar Alan (Değişkenlik)')">❄️ Polar Alan (Değişkenlik)</a></li>
                                        
                                        <li><hr class="dropdown-divider border-secondary"></li>
                                        <li><h6 class="dropdown-header text-dim">Diğer</h6></li>
                                        <li><a class="dropdown-item" href="javascript:void(0)" onclick="changeChartType('line', '📉 Çizgi Grafik (Trend)')">📉 Çizgi Grafik (Trend)</a></li>
                                        <li><a class="dropdown-item" href="javascript:void(0)" onclick="changeChartType('doughnut', '🍩 Örneklem Oranı (N)')">🍩 Örneklem Oranı (N)</a></li>
                                    </ul>
                                </div>
                            </div>
                                ...
                                <div class="row g-3">
                                    ...
                                    <div class="col-12 mt-3">
                                        <div class="p-3 rounded" style="background: rgba(99, 102, 241, 0.1); border: 1px solid rgba(99, 102, 241, 0.3);">
                                            <div class="metric-label text-white">İstatistik Kararı</div>
    ...
    <script>
        ...
        const revealObserver = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    ...
                    // Add stagger effect
                    const delay = Array.from(document.querySelectorAll('.reveal')).indexOf(entry.target) * 100;
                    setTimeout(() => {
        ...
        // Server-Side PDF Generation with Client-Side Chart Capture
        function preparePdfDownload() {
            const btn = document.getElementById('btnPdf');
            const txt = document.getElementById('btnPdfText');
            const spin = document.getElementById('btnPdfSpinner');
            
            // UI Feedback
            btn.disabled = true;
            txt.innerText = 'Grafikler İşleniyor...';
            spin.classList.remove('d-none');

            // 1. Switch Charts to Light Mode for better PDF appearance
            const originalConfigs = [];
            
            Chart.helpers.each(Chart.instances, function(instance) {
                // Save simplified state to restore later (optional, effectively we just toggle colors)
                const opts = instance.options;
                
                // We'll trust that hard-setting these to black will work, 
                // and we can revert them if we wanted, but for now let's just do one-way or simple toggle.
                // To restore properly, we'd need to store the previous values. 
                // For speed, let's just set them to Light.
                ...
                // Force white background for the image capture (Canvas transparent by default)
                // We can plugin a background color before drawing? 
                // Or just rely on PDF white paper. 
                // Chart.js produces transparent PNG by default. PDF is white. Black text on Transparent on White = OK.
                
                instance.update();
            });

            // Allow render cycle to finish
            setTimeout(() => {
                const chartsData = {};
                
                // 2. Capture Images
                Chart.helpers.each(Chart.instances, function(instance) {
                    const id = instance.canvas.id;
                    // Provide a white background for the image to be safe? 
                    // toBase64Image('image/png', 1.0)
                    chartsData[id] = instance.toBase64Image(); 
                });

                // 3. Submit Form
                const input = document.getElementById('chartsInput');
                input.value = JSON.stringify(chartsData);
                ...
                setTimeout(() => {
                    btn.disabled = false;
                    txt.innerText = '📥 PDF Rapor İndir';
                    spin.classList.add('d-none');
                    
                    // Optional: Reload to restore dark theme
                    // location.reload(); 
                    // Or ask user?
                }, 2000);

            }, 800); // 800ms wait for animation/render
        }
    </script>
</body>
</html>